...
BASE_OUT_DIR = Path(r"F:\\project\\YProgram\\QuestionsFinal\\Questions_v3")
...
def escape_json_string_for_sql_json(obj: Any) -> str:
    """
    之前用于选项/代码的加强转义（保留多层反斜杠以满足前端/数据库可视层需求）。
    这里保留原逻辑（把 '\' -> '\\\\' 三重转义），继续用于 options、code 等字段。
    """
    if obj is None:
        return "NULL"
    j = json.dumps(obj, ensure_ascii=False)
    # triple-escape backslashes to preserve deeper escaping in SQL literal
    j = j.replace("\\", "\\\\\\")
    j = j.replace("'", "''")
    return f"'{j}'"
...
def escape_json_double_backslashes_for_sql(obj: Any) -> str:
    """
    新增：专门用于 hints 字段（按照你的要求，每个 '\' 左右要多加一个 '\'）
    - 将 JSON 文本中的 '\' 变为 '\\'（双重）
    - 再将单引号转为 SQL 的 '' 并包单引号返回
    """
    if obj is None:
        return "NULL"
    j = json.dumps(obj, ensure_ascii=False)
...
def process_single_unit(client: OpenAI, global_unit_index: int, theme: str, out_base: Path) -> dict:
    ...
    if choice_rows:
        ...
        for r in choice_rows:
            ...
            # ---------- 修改点：hints 使用双反斜杠转义函数 ----------
            hints_sql = escape_json_double_backslashes_for_sql(r["hints"])
