...
BASE_OUT_DIR = Path(r"F:\\project\\YProgram\\QuestionsFinal\\Questions_v4")
...
def escape_json_string_for_sql_json(obj: Any) -> str:
    ...
    # 1) 将 JSON 文本里的每一对反斜杠（表示原始的一个反斜杠）由 2 -> 4
    #    e.g. "\\\\" (两个字符 in j) -> "\\\\\\\\" (四字符 in j)
    j = j.replace("\\\\", "\\\\\\\\")  # literal: '\\\\' -> '\\\\\\\\'

    # 2) 对常见的单字母转义在前面再加一个反斜杠（\n -> \\n, \" -> \\\" 等）
...
# ----------------------------
def main():
    print("=== generate_and_export_sql_final_v4 START ===")
