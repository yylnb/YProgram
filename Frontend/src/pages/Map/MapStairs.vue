...
const svgVW = ref(100)   // 使用 0..100 作为 x 的百分比坐标系 -> 与 left: n.x% 对齐
const svgVH = ref(600)   // will be updated to match canvasHeight in nodesPositions
...
const nodesPositions = computed(() => {
  // 每个阶段 30 个单元
  const baseUnitStart = Number(selectedStageInternal.value) * 30 + 1
  const arr = []
  const count = 30
  const gap = verticalSpacing
  const topOffset = 80

  // 布局仍沿用三列（左/中/右）的视觉样式 — 如需更多列可再调整
  ...
  // 与节点布局保持一致：canvasHeight 更新应包含最后一个节点的高度
  canvasHeight.value = topOffset + (count - 1) * gap + 120
  // 把 svg 的高度设置为 canvas 高度（像素），保证 SVG 的 y 坐标与节点 top: n.y px 对齐
  svgVH.value = Math.max(canvasHeight.value, 600)
  // svgVW 保持为 100（0..100 对应百分比 x）
  svgVW.value = 100
...
// -----------------------------
// svg helpers (unchanged except coordinate mapping uses same units as nodesPositions)
...
const fullPath = computed(() => {
  ...
  // 使用与 DOM 节点相同坐标：x 为 0..100（对应 left: x%），y 为像素值（与 canvasHeight 对齐）
...
function simpleTitle(localNumber) {
  // 每阶段 30 关：把序号显示为 1..30（循环）
  const idx = ((localNumber - 1) % 30) + 1
  return `第 ${idx} 关`
}
...
.state-unlocked {
  ...
  box-shadow:
    0 0 32px rgba(99, 101, 241, 0.49),
    6px -6px 0 0 rgba(165, 164, 164, 0.459);
}
