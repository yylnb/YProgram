<template>
  <div class="library-root">
        ...
        <template v-for="topic in visibleTopics" :key="topic.id">
          <article class="card p-4">
            <div class="flex justify-between items-start gap-3">
              ...
              <div class="actions">
                <!-- 查看按钮：现在只触发打开 detail 的动作（不在 Main 请求详情） -->
    ...
    <!-- LibraryDetail: 由 Main 控制 visible + 传入 language/topicId -->
    <!-- 注意：LibraryDetail.vue 需要实现 `visible` prop 并 emit 'update:visible' 或支持 v-model:visible -->
    <LibraryDetail
...
import LibraryDetail from './LibraryDetail.vue' // 细节弹窗组件（稍后实现）
...
// detail control (Main only manages which topic is selected + whether detail is visible)
...
function openTopic(id) {
  ...
  selectedTopicId.value = nid
  detailVisible.value = true
  // note: LibraryDetail 应负责加载详情数据（或接收 topic 对象），Main 不再直接请求详情
}
...
/* ---------- modal 样式（保留以供 LibraryDetail 使用） ---------- */
